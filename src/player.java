...
public class player  {
	...
	public void setNow()
	{
		...
		{
		  case 0: 
			  if(wuditime==0){s=ps;w=pw;a=pa;d=pd;break;}
			  else {s=sps;w=spw;a=spa;d=spd;break;}			  
		  case 1: 
			  if(wuditime==0){s=fengs;w=fengw;a=fenga;d=fengd;break;}
			  else {s=sfengs;w=sfengw;a=sfenga;d=sfengd;break;}
		  case 2:
			  if(wuditime==0) {s=paos;w=paow; a=paoa;d=paod;break;}
			  else   {s=spaos;w=spaow; a=spaoa;d=spaod;break;}
		  case 3: 
			  if(wuditime==0){s=guis; w=guiw; a=guia; d=guid; break;}
			  else{s=sguis; w=sguiw; a=sguia; d=sguid; break;}
		  case 4: 
			  if(wuditime==0){s=zhus;w=zhuw; a=zhua;d=zhud;break;}
			  else {s=szhus;w=szhuw; a=szhua;d=szhud;break;}
		  case 5: 
			  s=pkunzhu;w=pkunzhu; a=pkunzhu;d=pkunzhu;break;
		  case 6: 
			 s=pdie; w=pdie;a=pdie;d=pdie;break;
			
		  case 7:
			  s=pwin;w=pwin; a=pwin; d=pwin; break;			  
		}
	}
	public void move()
    {
    	...
    	{
    		if(now==s) {setNow();now=s;}
    		if(now==w) {setNow();now=w;}
    		if(now==a) {setNow(); now=a;}
    		if(now==d) {setNow();now=d;}
    	}
		setNow();
		if(yangzi==0||yangzi==1||yangzi==2||yangzi==4)
		{
			if(yangzi==0||yangzi==2||yangzi==4) speed=nspeed;
	    	if(RIGHT==true&&dRIGHT==0) { now=d;if(cangod()) X+=speed;return; }
	    	if(LEFT==true&&dLEFT==0)  {now=a;if(cangoa())X-=speed; return;}
	    	if(DOWN==true&&dDOWN==0)  { now=s;if(cangos())Y+=speed;return;}
	    	if(UP==true&&dUP==0)  { now=w;if(cangow())Y-=speed;return;}    	
		}
		if(yangzi==3)
		{
	    	if(LEFT==true&&dRIGHT==0) {if(cangod()) X+=speed;now=d;return; }
	    	if(RIGHT==true&&dLEFT==0)  {if(cangoa())X-=speed;now=a; return;}
	    	if(UP==true&&dDOWN==0)  {if(cangos())Y+=speed; now=s;return;}
	    	if(DOWN==true&&dUP==0)  {if(cangow())Y-=speed; now=w;return;}
		}
    }
	...
	public void toDie(player p)
	{
		...
		if(yangzi==5)
		{
			...
			if(dietime==Dietime&&p==BattleCanvas.p1) 
				{Die();BattleCanvas.p2.Win();}
			if(dietime==Dietime&&p==BattleCanvas.p2) 
			    {Die();BattleCanvas.p1.Win();}
			if(BattleCanvas.p1.yangzi!=5&&p==BattleCanvas.p2&&BattleCanvas.p1.getHeng()==BattleCanvas.p2.getHeng()&&BattleCanvas.p1.getShu()==BattleCanvas.p2.getShu()) 
				{BattleCanvas.p2.Die();BattleCanvas.p1.Win();}
			if(BattleCanvas.p2.yangzi!=5&&p==BattleCanvas.p1&&BattleCanvas.p1.getHeng()==BattleCanvas.p2.getHeng()&&BattleCanvas.p1.getShu()==BattleCanvas.p2.getShu()) 
				{BattleCanvas.p1.Die();BattleCanvas.p2.Win();} 
		}
	}
	public void Die()
	{
	...
	public void beZhu(player p)
	{
				...
				BattleCanvas.p2.yangzi==2||BattleCanvas.p2.yangzi==3||BattleCanvas.p2.yangzi==4)) 
		{
			...
			if(BattleCanvas.p2.now==BattleCanvas.p2.s) {BattleCanvas.p2.setNow();BattleCanvas.p2.now=BattleCanvas.p2.s;}
			if(BattleCanvas.p2.now==BattleCanvas.p2.w) {BattleCanvas.p2.setNow();BattleCanvas.p2.now=BattleCanvas.p2.w;}
			if(BattleCanvas.p2.now==BattleCanvas.p2.a) {BattleCanvas.p2.setNow();BattleCanvas.p2.now=BattleCanvas.p2.a;}
			if(BattleCanvas.p2.now==BattleCanvas.p2.d) {BattleCanvas.p2.setNow();BattleCanvas.p2.now=BattleCanvas.p2.d;}
				...
				BattleCanvas.p1.yangzi==2||BattleCanvas.p1.yangzi==3||BattleCanvas.p1.yangzi==4)) 
		{
			...
			if(BattleCanvas.p1.now==BattleCanvas.p1.s) {BattleCanvas.p1.setNow();BattleCanvas.p1.now=BattleCanvas.p1.s;}
			if(BattleCanvas.p1.now==BattleCanvas.p1.w) {BattleCanvas.p1.setNow();BattleCanvas.p1.now=BattleCanvas.p1.w;}
			if(BattleCanvas.p1.now==BattleCanvas.p1.a) {BattleCanvas.p1.setNow();BattleCanvas.p1.now=BattleCanvas.p1.a;}
			if(BattleCanvas.p1.now==BattleCanvas.p1.d) {BattleCanvas.p1.setNow();BattleCanvas.p1.now=BattleCanvas.p1.d;}
					...
					Battlemaoxian.p1.yangzi==2||Battlemaoxian.p1.yangzi==3||Battlemaoxian.p1.yangzi==4)) 
	{
		...
		if(Battlemaoxian.p1.now==Battlemaoxian.p1.s) {Battlemaoxian.p1.setNow();Battlemaoxian.p1.now=Battlemaoxian.p1.s;}
		if(Battlemaoxian.p1.now==Battlemaoxian.p1.w) {Battlemaoxian.p1.setNow();Battlemaoxian.p1.now=Battlemaoxian.p1.w;}
		if(Battlemaoxian.p1.now==Battlemaoxian.p1.a) {Battlemaoxian.p1.setNow();Battlemaoxian.p1.now=Battlemaoxian.p1.a;}
		if(Battlemaoxian.p1.now==Battlemaoxian.p1.d) {Battlemaoxian.p1.setNow();Battlemaoxian.p1.now=Battlemaoxian.p1.d;}
	...
	public void eatdaoju()
	{
		if(Map.isDaoju(getHeng(),getShu())) 
		{
			...
			{
			  case  1:{addAmount();break;}
			  case  2:{addAmount();break;}
			  case  3:{addPower();break;}
			  case  4:{addPower();break;}
			  case  5:{addSpeed();break;}
			  case  6:{addSpeed();break;}
			  case  7:{addFork();break;}
			  case  8:{beFeng();break;}
			  case  9:{bePao();break;}
			  case  10:{beGui();break;}
			  case  11:{beZhu(this);break;}
			} 
	...
	public void beExp()
	{
		...
		if( Map.isExp(getHeng(),getShu())&&(yangzi==1||yangzi==2||yangzi==3||yangzi==4)&&wuditime==0) 
			{bianshentime=MAXbianshen+1;beBack();}
	}
