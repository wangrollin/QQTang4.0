...
public class Battlemaoxian extends JPanel {
	...
	public void paintComponent (  Graphics page) 
	{
        ...
        if(p6.beexp()==true)
		{Music.stop();Music.music[8].loop(); Mypanel.lay.show(Play.panel,"zhanwin");}
		...
		//不断刷新的		
		p1.move();
		if(p1.toDie()) {Mypanel.lay.show(Play.panel,"zhanshu");}
		p1.eatdaoju();
		p1.beExp();
		p1.beBack();
		...
		else if(safe==-1)
		{
			...
			else if(p6.findway()==0) {safe=p6.safecheck();}
		...
		else 
		{
			...
			if(p6.getaway()==0){}
			else if(p6.getaway()==1){p6.goW();}
			else if(p6.getaway()==2){p6.goA();}
			else if(p6.getaway()==3){p6.goS();}
			else if(p6.getaway()==4){p6.goD();}
			safe=p6.safecheck();
		}
		////////////////////////////////////////////////////////////////
