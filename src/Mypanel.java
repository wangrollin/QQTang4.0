...
public class Mypanel extends JPanel implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()== CHOOSE.youxi)
		{
			  ...
			  if(Play.moshi==1) 
			  {
				  lay.show(this,"fight2");Play.zanting=1;;Music.music[11].loop();	
					addKeyListener(Battlemaoxian.p1);
				  }
			  if(Play.moshi==3){ 
				  lay.show(this,"fight3");Music.music[3].loop();
		...
		if (e.getSource()==  choose.biwu)
		{
			...
			which=1;
			Wallmap.biwu1();Map.dimian=Map.dimian1;
		}
		...
		if (e.getSource()==  choose.shuimian)
		{
			...
			which=2;
			Wallmap.putong();Map.dimian=Map.dimian2;
		}
		if (e.getSource()==  CHOOSE.kuangdong)
		{
			...
			for(int j=0;j<BattleCanvas.shu;j++)
				   for(int i=0;i<BattleCanvas.heng;i++)	
					   Map.wallmap[i][j]=null;
			which=3;
			Wallmap.maoxian();Map.dimian=Map.dimian3;
		}
